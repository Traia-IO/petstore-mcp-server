# Petstore MCP Server Implementation Guide

You are working on implementing the Petstore MCP (Model Context Protocol) server. The basic structure has been set up, and your task is to implement the actual API integration.
...
```json
{
  ...
  "tags": ["petstore", "api", /* add relevant tags like "search", "data", etc. */]
}
```
...
Add functions to call the Petstore API with retry support. Example pattern:
...
def call_petstore_api(endpoint: str, params: Dict[str, Any], api_key: str) -> Dict[str, Any]:
    """Call Petstore API endpoint with automatic retry."""
...
"Content-Type": "application/json"
    ...
    try:
        ...
        raise Exception(f"Petstore API error: {str(e)}")
```
...
```python
@mcp.tool()
async def search_petstore(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    """
    Search Petstore for [specific data type].
    ...
    try:
        # The call_petstore_api function already has retry logic
        results = call_petstore_api(
...
```python
@mcp.tool()
async def create_petstore_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Create a new item in Petstore.
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_petstore(query="example", limit=5)
   ```
...
- Check the Petstore API documentation: 
- Review the MCP specification: https://modelcontextprotocol.io
- Look at other MCP server examples in the Traia-IO organization

Remember: The goal is to make Petstore's capabilities easily accessible to AI agents through standardized MCP tools. 