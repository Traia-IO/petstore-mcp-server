#!/usr/bin/env python3
"""
Petstore MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('petstore_mcp')
...
logger.info("="*80)
logger.info(f"Petstore MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://petstore3.swagger.io/api/v3")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("Petstore MCP Server", host="0.0.0.0")
...
# - PETSTORE_API_KEY: Server's internal API key for payment mode
#
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="petstore-mcp-server"  # MCP server ID for tracking
    )
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Petstore MCP Server")
    logger.info("="*80)
